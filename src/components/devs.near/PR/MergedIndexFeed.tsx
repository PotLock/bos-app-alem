...
const MergedIndexFeed = (props: any) => {
  if (!props.index) {
    return "props.index is not defined";
  }
  const indices = JSON.parse(JSON.stringify(Array.isArray(props.index) ? props.index : [props.index]));
  const requiredIndices = indices.filter((index: any) => index.required);
  ...
  const computeFetchFrom = (items: any, limit: any, desc: any) => {
  ...
  function mergeItems(iIndex: any, oldItems: any, newItems: any, desc: any) {
    ...
    mergedItems.sort((a, b) => (desc ? b.blockHeight - a.blockHeight : a.blockHeight - b.blockHeight));
    return mergedItems;
  }
  ...
  if (requiredIndices.length > 0) {
    ...
    commonUniqueIdentifiers =
      itemsByRequiredIndex.length && itemsByRequiredIndex.reduce((a, b) => a.filter((c: any) => b.includes(c)));
  }
  ...
  const fetchMore =
    props.manual &&
    (state.feeds.some((f: any) => !!f.fetchFrom) && filteredItems.length < state.displayCount
      ? loader
