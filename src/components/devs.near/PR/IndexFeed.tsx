...
const IndexFeed = (props: any) => {
  ...
  const computeFetchFrom = (items: any, limit: any) => {
  ...
  const mergeItems = (newItems: any) => {
    const items = [...new Set([...newItems, ...state.items].map((i) => JSON.stringify(i)))].map((i) => JSON.parse(i));
  ...
  if (filter) {
    if (filter.ignore) {
      filteredItems = filteredItems.filter((item: any) => !(item.accountId in filter.ignore));
    }
  }
