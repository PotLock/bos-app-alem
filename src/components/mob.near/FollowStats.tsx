...
const FollowStats = (props: any) => {
  ...
  const followers = Social.keys<any>(`*/graph/follow/${accountId}`, "final", {
    return_type: "BlockHeight",
    values_only: true,
  });
  ...
  return (
    <div>
      <div className="d-flex flex-row">
        <div className="me-4">
          <a href={`#/mob.near/widget/FollowPage?accountId=${accountId}&tab=following`} className="text-dark">
            {numFollowing !== null ? <span className="fw-bolder">{numFollowing}</span> : "?"}{" "}
            <span className="text-muted">Following</span>
          </a>
        </div>
        <div>
          <a href={`#/mob.near/widget/FollowPage?accountId=${accountId}&tab=followers`} className="text-dark">
            {numFollowers !== null ? <span className="fw-bolder">{numFollowers}</span> : "?"}{" "}
