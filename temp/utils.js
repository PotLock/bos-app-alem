...
const utils = {
  ...
  validateGithubRepoUrl: (url) => {
    ...
    const githubRepoUrlPattern = new RegExp(/^(https?:\/\/)?(www\.)?github\.com\/[a-zA-Z0-9-]+\/[a-zA-Z0-9_.-]+\/?$/);
  ...
  yoctosToUsd: (amount) => {
    return nearToUsd ? "~$" + formatWithCommas(new Big(amount).mul(nearToUsd).div(1e24).toFixed(2)) : null;
  },
